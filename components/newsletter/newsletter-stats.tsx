...
export function NewsletterStats() {
  ...
  const exportToCSV = (subscriptions: Subscription[], filename: string) => {
    const headers = ['Email', 'Language', 'Status', 'Subscribed At', 'Unsubscribed At']
    const rows = subscriptions.map(sub => [
      sub.email,
      sub.locale === 'zh' ? '中文' : 'English',
      sub.isActive ? '活跃' : '已取消',
      new Date(sub.subscribedAt).toLocaleString('zh-CN'),
      sub.unsubscribedAt ? new Date(sub.unsubscribedAt).toLocaleString('zh-CN') : ''
    ])
  ...
  const handleExportActive = () => {
    exportToCSV(activeSubscriptions, `newsletter_active_${new Date().toISOString().split('T')[0]}.csv`)
  }

  const handleExportAll = () => {
    exportToCSV(subscriptions, `newsletter_all_${new Date().toISOString().split('T')[0]}.csv`)
  }
